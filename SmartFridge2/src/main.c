...
void default_alimenti(){
	...
	if(apriFile(&file_alimenti, FILENAME_ALIMENTI, "rb+")){
		...
		alimento.quantita = 2500;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//2° alimento
		strcpy(alimento.nome, "PASTA SFOGLIA");
		...
		//4° alimento
		strcpy(alimento.nome, "PROSCIUTTO CRUDO");
		alimento.unita = PESO_GR;
		alimento.quantita = 200;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//5° alimento
		strcpy(alimento.nome, "PROSCIUTTO COTTO");
		alimento.unita = PESO_GR;
		...
		//8° alimento
		strcpy(alimento.nome, "PANNA FRESCA");
		alimento.unita	= PESO_ML;
		...
		//17° alimento
		strcpy(alimento.nome, "FETTINA DI VITELLO");
		alimento.unita = PESO_GR;
		alimento.quantita = 200;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//18° alimento
		strcpy(alimento.nome, "COSCIE DI POLLO");
		alimento.unita = N_UNIT;
		alimento.quantita = 6;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//19° alimento
		strcpy(alimento.nome, "PETTO DI POLLO");
		alimento.unita = PESO_GR;
		alimento.quantita = 600;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//20° alimento
		strcpy(alimento.nome, "FILETTO DI TONNO");
		alimento.unita = PESO_GR;
		alimento.quantita = 200;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//20° alimento
		strcpy(alimento.nome, "FILETTO DI PESCE SPADA");
		alimento.unita = PESO_GR;
		alimento.quantita = 250;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//21° alimento
		strcpy(alimento.nome, "MOZZARELLA DI BUFALA");
		alimento.unita = PESO_GR;
		alimento.quantita = 600;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//22° alimento
		strcpy(alimento.nome, "MOZZARELLA FIORDILATTE");
		alimento.unita = PESO_GR;
		alimento.quantita = 400;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//23° alimento
		strcpy(alimento.nome, "PASTA PER PIZZA");
		alimento.unita = N_UNIT;
		alimento.quantita = 2;
		fwrite(&alimento, sizeof(t_alimento), 1, file_alimenti);

		//24° alimento
		strcpy(alimento.nome, "SALSA DI POMODORO");
		alimento.unita = PESO_ML;
...
void default_ricette(){
	...
	if(apriFile(&file_ricette, FILENAME_RICETTE, "rb+")){
		...
		strcpy(ricetta.procedimento, "Condisci la pasta sfoglia con ricotta e spinaci, dopo metti in forno e dopo 20 min è pronta ");
		...
		strcpy(ricetta.procedimento, "Svuota le melanzane e riempi con parmigiano  e pomodorini, metti in forno per 20 min ed è pronta");
	 ...
	 strcpy(ricetta.procedimento, "Taglia il petto di pollo a cubetti, scoglio burro il burro insieme al latte e al curry,\n"
								"quando il burro sarà completamente fuso aggiungi il pollo e in 10 min sarà pronto");
