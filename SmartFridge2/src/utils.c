...
void isPositive(float quantita,int* flag_errore){
  *flag_errore = 0;
  ...
  if (quantita <= 0){
    *flag_errore = 1;
    printf("\tERRORE! numero minore o uguale a 0\n");
  }
}
...
void checkFloat(float quantita_float, int *flag_errore) {
  ...
  *flag_errore = 0;
  ...
  quantita_int = (int)quantita_float; // viene fatto il cast(int), quindi se per esempio:
  ...
  if (str[0] == '.' || str[strlen(str) - 1] == '.') {
    *flag_errore = 1;
    ...
    while ((i < len) && (*flag_errore == 0)) {
    	  // possono esserci solo numeri
		  // ci deve essere solo un punto
		  // il punto non puo' essere ne' all inizio ne' alla fine
		  // il trattino può essere solo in prima posizione
